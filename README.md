...
图片增强会将候选页的完整内容上传至 MinerU，需要配置 Token。按单页提交；提取后清理本页未使用的共享图片/字体等资源，再压缩保存，并在上传前验证文件只有 1 页。日志显示原文页码、提取页数和清理后大小。旧缓存中尚未完成的单页文件会重新清理，已完成任务继续复用。成功结果缓存于输出根目录 `.figure_cache`，源文件内容、模型或语言变化会使用新缓存。失败会报错并保留缓存，不发布新的完成输出；识别结果没有图片时明确提示核查。缓存可能包含源页面和识别结果，应按原文档保管。

原生位图边界用于修正 MinerU 裁框，避免裁掉脚注。图片区域内文字不再重复输出；MinerU 图注和正文不替换原生文字。候选检测覆盖图题与较明显的矢量图形组，不能保证无图题小图全部检出。旋转候选页暂不支持；跨页图、复杂裁剪及多图版式仍需人工核查。新增运行模块 `figure_enrichment.py` 必须与主程序一起更新。

图片增强同时处理 MinerU 的 `image` 和 `chart` 区域；带明确上方 Figure 图题的表格区域也作为插图渲染，避免丢失被误分类的时序图、位域图。使用原生文字边界补全附近图内标签，保护正文和编号图注，合并重叠图片区域；被替换的旧图片不会残留在最终 images 目录。
...
程序首先判断 PDF 是否包含足够的可见原生文字。原生文字 PDF 的正文在本地处理；默认图片增强会上传候选图片页的完整内容，完全禁止上传需指定 `--route native` 或 `--disable-mineru-ocr`；整页扫描图、隐藏 OCR 文字层或原生文字不足的 PDF 会自动调用 MinerU 精准解析 API，并强制开启 OCR。原生解析会读取：
...
- 带 `Fig.` 标题的纯矢量论文插图会渲染到 `images`；默认使用 MinerU 辅助定位，本地模式采用原生图形规则。
...
3. PDF 具有足够的可见原生文字、没有检测到整页扫描页，并且没有超复杂矢量页面时，正文使用本地原生元素解析，默认对候选图片页调用 MinerU 增强。
...
- 默认图片增强会上传候选页，MinerU OCR 路由会上传整份文档（可能分片）；敏感文件请使用 `--route native` 或 `--disable-mineru-ocr`。
