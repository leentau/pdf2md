...
5. 任意页面的压缩绘图内容超过 2 MiB 时，为避免本地展开数十万个矢量路径而卡住，整份 PDF 自动使用 MinerU OCR。
...
- 单页压缩绘图内容超过 2 MiB 时，默认的 `--route auto` 会把整份 PDF 交给 MinerU OCR；这会将文档上传到 MinerU。使用 `--route native` 或 `--disable-mineru-ocr` 可以禁止上传，但程序会停止并说明触发自动分流的页码和大小。
