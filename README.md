...
大文件 OCR 的进度保存在输出根目录的 `.mineru_cache`。网络中断或程序退出后，再运行同一条命令会从已上传/已完成的分片继续；结果 ZIP 下载中断时也会通过 HTTP Range 从 `.part` 文件继续。下载过程每增加约 8 MiB 显示一次进度，连续 180 秒没有收到数据才会超时重试；文件上传连续无响应的超时同样为 180 秒。全部完成后缓存自动删除。分片合并不会插入源 PDF 中不存在的 Markdown 分隔线，所有图片会重命名后汇总到最终 `images` 文件夹。
...
- 单页压缩绘图内容超过 2 MiB 时，程序会跳过该页的矢量插图和矢量表格分析，避免数十万个路径导致转换卡住；正文、标题和嵌入图片仍会正常输出，终端会显示对应警告。
